...
def smile(x, y, color):
    ...
    sd.ellipse(left_bottom=left_bottom, right_top=right_top, width=4, color=color)
    point = sd.get_point(x - 20, y + 10)
    sd.circle(center_position=point, radius=5, width=0, color=color)
    point = sd.get_point(x + 20, y + 10)
    sd.circle(center_position=point, radius=5, width=0, color=color)
    ...
    sd.lines(point_list=point_list, width=4, color=color)
